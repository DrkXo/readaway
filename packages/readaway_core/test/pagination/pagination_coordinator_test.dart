...
void main() {
  group('PaginationCoordinator', () {
    ...
    test('setCurrentAnchor restores the global page', () {
      coordinator.initialize(
        chapterCount: 2,
        viewportHeight: 800,
        contentHeight: 1600,
      ...
      expect(state.pageInChapter, 0);
      expect(state.globalPage, 2);
      expect(coordinator.currentAnchor.chapterIndex, 1);
    });

    test('anchor survives viewport reflow', () {
      coordinator.initialize(
        chapterCount: 1,
        viewportHeight: 800,
        contentHeight: 1600,
      ...
      expect(coordinator.currentState.globalPage, 1);

      // Reflow (e.g. font-size change) recomputes pagination but keeps the anchor.
      coordinator.updateViewport(viewportHeight: 400, contentHeight: 1600);
      expect(
        coordinator.currentAnchor.progressionInChapter,
        closeTo(0.5, 0.001),
      );
      expect(coordinator.currentState.globalPage, 2);
    });

    test('offsetForAnchor and anchorForOffset round-trip', () {
      coordinator.initialize(
        chapterCount: 2,
        viewportHeight: 800,
        contentHeight: 1600,
      ...
      final anchor = coordinator.anchorForOffset(1, 400);
      expect(anchor.chapterIndex, 1);
      expect(anchor.progressionInChapter, closeTo(0.5, 0.001));

      final offset = coordinator.offsetForAnchor(anchor);
      expect(offset, closeTo(1600 + 400, 0.001));
    });

    test('offsetForAnchor returns null for unmeasured chapters', () {
      coordinator.initialize(
        chapterCount: 2,
        viewportHeight: 800,
        contentHeight: 1600,
