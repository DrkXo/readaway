...
class PaginationCoordinator {
  ...
  /// The current stable reading position (chapter + progression).
  ...
  /// Sets the current reading position from a stable [ReadingAnchor].
  ///
  /// The anchor is kept as the source of truth, so the position survives
  /// reflow changes (font-size, window resize) until the user navigates.
  ...
  /// Computes the cumulative scroll offset (in pixels) for a [ReadingAnchor]
  /// based on registered chapter heights.
  ///
  /// Returns `null` when the target chapter's height has not been measured yet.
  ...
  /// Creates a [ReadingAnchor] from a scroll offset within a chapter.
  ///
  /// [offsetInChapter] is the scroll offset relative to the top of the chapter.
  /// The progression is clamped to `[0, 1]`.
